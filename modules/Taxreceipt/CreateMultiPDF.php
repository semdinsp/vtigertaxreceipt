...
if(isset($_REQUEST['savemode']) && $_REQUEST['savemode'] == 'file')
{
	...
	$filepath='test/product/'.$taxreceipt_id.'_MultiTaxReceipt.pdf';
	$pdf->Output($filepath,'F'); //added file name to make it work in IE, also forces the download giving the user the option to save

}else {
	$pdf->Output('MultiTaxReceipt.pdf','D'); //added file name to make it work in IE, also forces the download giving the user the option to save
	exit();
}
